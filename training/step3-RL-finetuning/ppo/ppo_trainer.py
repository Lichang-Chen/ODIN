...
class DeepSpeedPPOTrainer():
    ...
    def __init__(self, rlhf_engine, args):
        ...
        self.cliprange = args.pratio_cliprange
        self.cliprange_value = 0.2
        self.gamma = args.gamma
        self.lam = 0.95
        self.generate_time = 0.0

    def _generate_sequence(self, prompts, mask, step, do_sample=True):
    ...
    def generate_experience(self, prompts, mask, step, training_mode=True):
        self.eval()
        ...
        with torch.no_grad():
            if training_mode:
                ...
                output = self.actor_model(seq, use_cache=False, attention_mask=attention_mask)
                output_ref = self.ref_model(seq, use_cache=False, attention_mask=attention_mask)
    ...
    def compute_rewards(self, prompts, log_probs, ref_log_probs, reward_score,
        ...
        for j in range(batch_size):
            rewards[j, start:ends[j]][-1] += reward_clip[j] # the remax implementation: reward * self.gamma

        return rewards

    def train_rlhf(self, inputs, ignore_actor_update=False):

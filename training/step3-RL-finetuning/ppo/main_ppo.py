...
def parse_args():
    ...
    parser.add_argument('--global_step', type=int, default=0, help='Control the global step')
    parser.add_argument('--exit_duration_in_mins', type=float, default=float('inf'))
    parser.add_argument('--actor_delay', type=int, default=-1,
                        help='Delay actor update until the critic has been warmed up properly.')
    parser.add_argument('--pratio_cliprange', type=float, default=0.2,
...
def get_dsp_save_path(args, model="actor"):
    assert model in ["actor", "critic"]
    if model == "actor":
        return os.path.join(args.output_dir, 'dsp_state', 'actor')
    elif model == "critic":
        return os.path.join(args.output_dir, 'dsp_state', 'critic')
    else:
        NotImplementedError
...
def maybe_evaluate_and_save(
    ...
    print_rank_0(
        f"***** Evaluating policy, Epoch {epoch + 1}/{args.num_train_epochs} Step {n_grad_updates}/{len(train_dataloader) // args.gradient_accumulation_steps} *****",
        args.global_rank,
    )
    eval_reward, eval_length, eval_kl, eval_entropy = evaluation_by_reward(
        trainer, prompt_eval_dataloader, device, args, n_grad_updates, False
    )
    print_rank_0(
        f"eval reward: {eval_reward} | eval length: {eval_length} | eval kl: {eval_kl} | eval entropy: {eval_entropy}",
        args.global_rank,
    )
    ...
    if global_steps > 1:
        save_model(rlhf_engine, tokenizer, args, subfolder=f"iter_{n_grad_updates}", save_opt_states=False)
            ...
            if exp_dataset is not None:
                ...
                for ppo_ep in range(args.ppo_epochs):
                    for i, (exp_data, unsup_data) in enumerate(
                        ...
                        actor_loss, critic_loss = trainer.train_rlhf(exp_data, ignore_actor_update=global_step < args.actor_delay)
                ...
                n_grad_updates = rlhf_engine.actor.global_steps
                print_rank_0(
                    f'Epoch: {epoch} | Step: {n_grad_updates}/{len(prompt_train_dataloader) // args.gradient_accumulation_steps} | PPO Epoch: {ppo_ep+1} | Actor Loss: {actor_loss_sum/inner_iter} | Critic Loss: {critic_loss_sum/inner_iter} | Unsupervised Loss: {unsup_loss_sum/inner_iter}',
                    args.global_rank)

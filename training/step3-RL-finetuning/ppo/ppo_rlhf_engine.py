...
def log_init(model_name, stime=None):
    """Print the initialization stats of a given model."""
    if torch.distributed.get_rank() == 0:
        tag = "start" if stime is None else "end"
        suffix = "ing" if stime is None else "ed"
        duration = ""
        if stime is not None:
            duration = f"(duration: {time.time() - stime:.2f}s)"
...
class DeepSpeedRLHFEngine():
    """Modified implementation from the Deepspeed RLHF engine.
    We modified it to save and load optimizer states, so that we can resume properly if
    the job dies accidentally.
    """
    def __init__(self, actor_model_name_or_path, critic_model_name_or_path,
        ...
        self.actor_bf16 = args.actor_bf16
        self.critic_bf16 = args.critic_bf16
    ...
    def _init_actor(self, actor_model_name_or_path):
        ...
        # Optimizer
        adam_optimizer = DeepSpeedCPUAdam if self.args.offload else FusedAdam
        optim_params = get_optimizer_grouped_parameters(
            actor_model, self.args.actor_weight_decay,
            self.args.actor_lora_learning_rate)
        optim = adam_optimizer(optim_params,
    ...
    def _init_critic(self, critic_model_name_or_path):
        ...
        ds_eval_config = get_eval_ds_config(offload=False, bf16=self.args.critic_bf16,
                                            stage=self.args.critic_zero_stage)
        # We need to set train batch size and micro batch size here to pass
        # the sanity check of DeepSpeed engine.
        ds_eval_config[
            'train_micro_batch_size_per_gpu'] = self.args.per_device_training_batch_size
        ds_eval_config[
            'train_batch_size'] = (
        ...
        critic_model = create_critic_model(
            ...
            use_multihead=self.args.use_multihead,
            dcomp_config={
                "rm_num_heads": 2, "alpha": self.args.alpha,
                "use_quality_head": self.args.use_quality_head}
            )
    ...
    def _init_reward(self, model_name_or_path):
        stime = log_init("Reward")
        # DS Config
        zero_stage = 3

        ds_config = get_eval_ds_config(offload=self.args.offload, bf16=self.args.reward_bf16,
                                       stage=zero_stage)
        ds_config['train_micro_batch_size_per_gpu'] = self.args.per_device_training_batch_size
        ds_config[
            'train_batch_size'] = (
        ...
        ds_eval_config = get_eval_ds_config(offload=False, bf16=self.args.actor_bf16, stage=zero_stage)
        ...
        reward_model = create_critic_model(
            ...
            zero_stage=self.args.critic_zero_stage,
            use_multihead=self.args.use_multihead,
            dcomp_config={"rm_num_heads":2, "alpha": self.args.alpha, "use_quality_head": self.args.use_quality_head}
            )
    ...
    def save_optimization_states(self, dst_path):
        """Save the actor optimizer and lr_scheduler to dsp_path."""
        print(f"Saving actor optimizer and lr_scheduler state dict to {dst_path}")
        self.actor.save_checkpoint(os.path.join(dst_path, 'actor_optimizer_checkpoint'))
        self.critic.save_checkpoint(os.path.join(dst_path, 'critic_optimizer_checkpoint'))

    def try_load_optimization_states(self, src_path):
        """Load the optimizer states when they exist."""
        actor_path = os.path.join(src_path, 'actor_optimizer_checkpoint')
        critic_path = os.path.join(src_path, 'critic_optimizer_checkpoint')
        if os.path.exists(actor_path):
            assert os.path.exists(critic_path)
            self.actor.load_checkpoint(actor_path)
            self.critic.load_checkpoint(critic_path)
            print(f'Resumed optimizer and lr scheduler states from {src_path}')
        else:
            print('Actor and critic optimization states not found. Training from scratch.')

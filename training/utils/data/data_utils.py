...
def get_raw_dataset(dataset_name, output_path, seed, local_rank):
    ...
    if "Dahoas/rm-static" in dataset_name:
        ...
        return raw_datasets.DahoasFullhhrlhfDataset(output_path, seed,
                                                    local_rank, dataset_name)
    elif "local/oasst_multi_turn" in dataset_name:
        chat_path = os.path.abspath(os.path.join(os.path.dirname(__file__)))
        if not (os.path.isfile(chat_path + '/oasst_data/oasst_en_train_dialogue.json')):
            raise RuntimeError(
                f"Please check both the labels_oasst_en_train_dialogue.json files in your utils/data/oasst_data directory."
            )
        return raw_datasets.OASST_MULTI_TURN(output_path, seed, local_rank, dataset_name, chat_path)
    elif "local/oasst_prompt" in dataset_name:
        chat_path = os.path.abspath(os.path.join(os.path.dirname(__file__)))
        if not (os.path.isfile(chat_path + '/oasst_data/oasst_en_train_prompt.json')):
            raise RuntimeError(
                f"Please check both the weak_labels_oasst_en_train.json files in your utils/data/oasst_data directory."
            )
        return raw_datasets.OASST_PROMPT(output_path, seed, local_rank, dataset_name, chat_path)
...
class PromptDataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.train_phase == 1:
            ...
            # if 'labels' in self.prompt_dataset.keys():  #TODO
...
# Currently, it is only designed for stage 2: reward model training
class PromptDataset_Attributes(Dataset):

    def __init__(self, prompt_dataset, chosen_dataset, reject_dataset,
                 pad_token_id, chosen_labels, rejected_labels, train_phase=2) -> None:
        super().__init__()
        self.prompt_dataset = prompt_dataset
        self.chosen_dataset = chosen_dataset
        self.reject_dataset = reject_dataset
...
def create_dataset_split(current_dataset, raw_dataset, train_phase, tokenizer,
    ...
    if train_phase == 1:
        ...
        # reject_attributes_dataset = []
        # chosen_attributes_dataset = []
        for i, tmp_data in enumerate(current_dataset):
            # tokenize the text
            chosen_sentence = raw_dataset.get_prompt_and_chosen(
                tmp_data)  # the accept response
            reject_sentence = raw_dataset.get_prompt_and_rejected(
                tmp_data)  # the rejected response
        ...
        # return the promptdataset designed for reward models.
        return PromptDataset(prompt_dataset, chosen_dataset, reject_dataset, tokenizer.pad_token_id)
...
# class DataCollatorRewardwithAttributes:
...
#                                             [f[3] for f in data],
#                                             dim=0)
#         # TODO: add the different atttributes here.
#         device = batch['attention_mask'].device
#         batch['labels'] = torch.cat([torch.tensor(f[4]) for f in data] +
#                                     [torch.tensor(f[5]) for f in data],
#                                     dim=0
#                                     ).to(device)
#         return batch
...
class MiniDataset:
    ...
    def free(self):
        self.dataset = []
# generate the test cases for the dataset
